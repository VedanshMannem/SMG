...
export default function Navbar() {
  ...
  return user ? (
    ...
    // Top navbar when not logged in - MADE SMALLER
    <div className="fixed top-0 left-0 right-0 z-50 mx-4">
      <div className="mt-2 rounded-2xl shadow-md bg-gray-50">
        <div className="flex justify-between items-center px-4 py-2">
          <button className="text-lg font-semibold" onClick={() => handleHomeClick()}>
            Stock Market Game
          </button>
          <button onClick={loginWithGoogle} className="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors text-sm">
            Login
          </button>
        </div>
      </div>
